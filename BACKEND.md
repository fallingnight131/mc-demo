...
| 运行时 | **Node ≥ 23**(原生跑 TS) | 本机 v23.11;`node src/index.ts` 直接执行,**服务端零构建步骤**(strip-only 约束:不用 enum/namespace/**构造器参数属性**,import 带 `.ts` 扩展名) |
...
| DELETE | `/api/saves/0` | — | `204`(清档重开,无版本校验) | `401` |
...
- **游玩期推送**:`SaveManager.saveNow()` 落盘后经 `onSaved(json)` 钩子把载荷交给
  ...
  `200 {rev}` → meta={rev, pending:false};`409 {rev}` → toast 提示(另一设备已更新),
  保持本地 pending 继续游玩,**下次启动对账**(备份本机 + 云为准)—— 游玩中不热切换世界。
