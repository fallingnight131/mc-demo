...
function paintTool(img: ImageData, toolId: number): void {
  ...
  if (toolId === Tool.Pickaxe) {
  ...
  } else if (toolId === Tool.Axe) {
    // 斧头:铁斧头在左上(左缘为受光刃口),木柄斜向右下,MC 物品栏风格。
    // 铁斧头主体(中灰)
    dots(
      [
        [5, 0], [6, 0],
    ...
    // 刃口高光(左/上外缘)
    dots([[5, 0], [4, 1], [3, 2], [2, 3], [2, 4], [3, 5]], [228, 233, 239]);
    // 刃口次缘(中亮)
    dots([[6, 0], [5, 1], [4, 2], [3, 3], [3, 4], [4, 5]], [190, 196, 205]);
    // 斧头暗部(右下,近柄座)
    dots([[7, 1], [7, 2], [7, 3], [7, 4], [7, 5], [6, 6]], [104, 110, 120]);
    // 斧头深轮廓(左下底缘)
    dots([[3, 6], [4, 6]], [58, 61, 68]);
    // 木柄(斜向右下,2px)
    dots(
      [
        [7, 6], [8, 6], [8, 7], [9, 7], [9, 8], [10, 8],
        [10, 9], [11, 9], [11, 10], [12, 10], [12, 11], [13, 11],
        [13, 12], [14, 12], [14, 13], [15, 13],
      ],
      [110, 74, 42],
    );
    // 木柄亮面(上缘)
    dots([[8, 6], [9, 7], [10, 8], [11, 9], [12, 10], [13, 11], [14, 12], [15, 13]], [150, 104, 58]);
    // 木柄暗面(下缘)
    dots([[8, 7], [9, 8], [10, 9], [11, 10], [12, 11], [13, 12], [14, 13]], [78, 50, 28]);
  } else if (toolId === Equip.IronHelmet) {
    // 铁头盔:圆顶 + 护颊 + 目缝
    dots(
      [
    ...
    for (let y = 4; y <= 8; y++) for (let x = 4; x <= 11; x++) P(x, y, 172, 178, 188);
    dots([[5, 4], [6, 4], [7, 4], [4, 5], [5, 5], [4, 6]], [224, 230, 238]); // 顶部受光
    dots([[5, 8], [6, 8], [7, 8], [8, 8], [9, 8], [10, 8]], [40, 42, 48]); // 目缝
    dots([[6, 9], [7, 9], [8, 9], [9, 9]], [120, 126, 136]); // 鼻梁护板
  } else if (toolId === Equip.IronChest) {
    // 铁护胸:肩甲 + 胸板中脊
    dots(
      [
        [3, 3], [4, 3], [5, 3], [10, 3], [11, 3], [12, 3],
        [2, 4], [13, 4], [2, 5], [13, 5],
        [3, 6], [12, 6], [3, 7], [12, 7], [3, 8], [12, 8],
        [4, 9], [11, 9], [4, 10], [11, 10],
    ...
    for (let y = 4; y <= 8; y++) for (let x = 4; x <= 11; x++) P(x, y, 172, 178, 188);
    for (let y = 9; y <= 10; y++) for (let x = 5; x <= 10; x++) P(x, y, 160, 166, 176);
    dots([[4, 4], [5, 4], [10, 4], [11, 4], [3, 4], [12, 4]], [224, 230, 238]); // 肩甲受光
    dots([[7, 5], [8, 5], [7, 6], [8, 6], [7, 7], [8, 7], [7, 8], [8, 8], [7, 9], [8, 9], [7, 10], [8, 10]], [120, 126, 136]); // 中脊
  } else if (toolId === Equip.IronLegs) {
    // 铁护腿:腰带 + 两条胫甲
    dots([[4, 3], [5, 3], [6, 3], [7, 3], [8, 3], [9, 3], [10, 3], [11, 3]], [70, 74, 84]);
    for (let x = 4; x <= 11; x++) P(x, 4, 150, 156, 166);
    for (let y = 5; y <= 12; y++) {
      for (const x0 of [4, 9]) {
        P(x0, y, 70, 74, 84);
        P(x0 + 1, y, 172, 178, 188);
        P(x0 + 2, y, 132, 138, 148);
      }
    }
    dots([[4, 13], [5, 13], [6, 13], [9, 13], [10, 13], [11, 13]], [70, 74, 84]); // 靴口
    dots([[5, 5], [5, 6], [10, 5], [10, 6]], [224, 230, 238]);
  } else if (toolId === Equip.SwiftCharm) {
    // 疾风护符:金环 + 三道风羽
    dots([[11, 3], [12, 3], [10, 4], [13, 4], [10, 5], [13, 5], [11, 6], [12, 6]], [212, 168, 60]);
    dots([[11, 4], [12, 4], [11, 5], [12, 5]], [255, 224, 120]);
    // 风羽(左扫,三层)
    dots([[2, 6], [3, 6], [4, 6], [5, 6], [6, 6], [7, 6], [8, 6], [9, 7], [10, 7]], [225, 238, 248]);
    dots([[3, 8], [4, 8], [5, 8], [6, 8], [7, 8], [8, 9], [9, 9]], [190, 214, 234]);
    dots([[4, 10], [5, 10], [6, 10], [7, 11], [8, 11]], [150, 182, 212]);
  } else if (toolId === Equip.CloudBottle) {
    // 云朵瓶:软木塞玻璃瓶,内装一朵云
    dots([[6, 2], [7, 2], [8, 2], [9, 2], [6, 3], [9, 3]], [150, 104, 58]); // 木塞
    dots(
      [
        [6, 4], [9, 4], [5, 5], [10, 5],
        [4, 6], [11, 6], [4, 7], [11, 7], [4, 8], [11, 8],
        [4, 9], [11, 9], [4, 10], [11, 10],
        [5, 11], [10, 11], [6, 12], [7, 12], [8, 12], [9, 12],
      ],
      [136, 168, 196],
    );
    dots([[5, 6], [5, 7], [5, 8]], [200, 224, 244]); // 玻璃高光
    // 云
    dots([[6, 8], [7, 7], [8, 7], [9, 8], [6, 9], [7, 9], [8, 9], [9, 9], [7, 10], [8, 10]], [244, 248, 252]);
    dots([[6, 10], [9, 10]], [210, 220, 232]);
  } else if (toolId === Equip.Horseshoe) {
    // 幸运马蹄铁:金色 U 形 + 钉孔
    dots(
      [
        [5, 3], [6, 3], [7, 3], [8, 3], [9, 3], [10, 3],
