...
function paintTool(img: ImageData, toolId: number): void {
  ...
  if (toolId === Tool.Pickaxe) {
    // 镐子:顶部弧形金属镐头(两端收成尖)+ 中央颈部 + 斜向木柄,贴近 MC 物品栏像素画。
    // 镐头深色轮廓:顶边 + 两条向外下弯的弧臂(末端收成 1px 尖)+ 颈部两侧。
    dots(
      [
    ...
    // 木柄:斜向左下,每两行左移一列;两侧深轮廓,柄芯左亮右暗。
    const handle: Array<[number, number, number, number]> = [
      [6, 7, 8, 9], [6, 7, 8, 9],
      [5, 6, 7, 8], [5, 6, 7, 8],
      [4, 5, 6, 7], [4, 5, 6, 7],
      [3, 4, 5, 6], [3, 4, 5, 6],
